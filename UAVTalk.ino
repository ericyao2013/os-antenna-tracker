...
//Decoding an incoming UAVTalk Message.
		...
		if (uavtalk_parse_char(c, &msg)) {
			...
			switch (msg.ObjID) {
				...
				/*case ATTITUDEACTUAL_OBJID:
					...
					last_flighttelemetry_connect = millis();
					show_prio_info = 1;
        				veh_roll		= (int16_t) uavtalk_get_float(&msg, ATTITUDEACTUAL_OBJ_ROLL);
        				veh_pitch		= (int16_t) uavtalk_get_float(&msg, ATTITUDEACTUAL_OBJ_PITCH);
        				veh_yaw			= (int16_t) uavtalk_get_float(&msg, ATTITUDEACTUAL_OBJ_YAW);
				...
				/*case FLIGHTSTATUS_OBJID:
        				...
        				//veh_mode		= uavtalk_get_int8(&msg, FLIGHTSTATUS_OBJ_FLIGHTMODE);
        				if (veh_armed == 0 || veh_armed == 1) {
        				    motor_armed = 0;
        				}
        				else if (veh_armed == 2) {
        				    motor_armed = 1;
				...
				case MANUALCONTROLCOMMAND_OBJID:
                                        response = UAVTALK_TYPE_ACK;
					veh_throttle		= (int16_t) (100.0 * uavtalk_get_float(&msg, MANUALCONTROLCOMMAND_OBJ_THROTTLE));
					if (veh_throttle < 0 || veh_throttle > 200) veh_throttle = 0;
					// Channel mapping:
					// 0   is Throttle
                                        // 1-2 are Roll / Pitch 
                                        // 3   is Yaw
                                        ...
                                        // veh_chanx_raw used for panel navigation (Accessory)
					...
					veh_chan5_raw		= uavtalk_get_int16(&msg, MANUALCONTROLCOMMAND_OBJ_CHANNEL_4);
					veh_chan6_raw		= uavtalk_get_int16(&msg, MANUALCONTROLCOMMAND_OBJ_CHANNEL_6);
					veh_chan7_raw		= uavtalk_get_int16(&msg, MANUALCONTROLCOMMAND_OBJ_CHANNEL_7);
					veh_chan8_raw		= uavtalk_get_int16(&msg, MANUALCONTROLCOMMAND_OBJ_CHANNEL_8);
                                        
				break;*/
				case GPSPOSITION_OBJID:
                                        response = UAVTALK_TYPE_ACK;
					veh_lat			= uavtalk_get_int32(&msg, GPSPOSITION_OBJ_LAT) / 10000000.0;
					veh_lon			= uavtalk_get_int32(&msg, GPSPOSITION_OBJ_LON) / 10000000.0;
					veh_satellites_visible	= uavtalk_get_int8(&msg, GPSPOSITION_OBJ_SATELLITES);
					veh_fix_type		= uavtalk_get_int8(&msg, GPSPOSITION_OBJ_STATUS);
					veh_heading		= uavtalk_get_float(&msg, GPSPOSITION_OBJ_HEADING);
					veh_alt			= uavtalk_get_float(&msg, GPSPOSITION_OBJ_ALTITUDE);
					veh_groundspeed		= uavtalk_get_float(&msg, GPSPOSITION_OBJ_GROUNDSPEED);
				...
				// veh_waypoint_seq = 0;           // waypoint sequence
